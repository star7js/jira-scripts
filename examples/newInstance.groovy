...
// comment out next line unless testing in console, as issue will be available from context in Automation
def issue = Issues.getByKey("PRC-9")
...
// Define the template key based on the project type
def templateKey
if (typeKeyStr == "software") {
    templateKey = boardType == "scrum" ? "com.pyxis.greenhopper.jira:gh-scrum-template" : "com.pyxis.greenhopper.jira:gh-kanban-template"
} else if (typeKeyStr == "business") {
    templateKey = "com.atlassian.jira-core-project-templates:jira-core-simplified-project-management"
    // Adjust if needed for your specific business template
} else if (typeKeyStr == "service_desk") {
    templateKey = "com.atlassian.servicedesk:simplified-it-service-desk"
    // Example; confirm the exact key for your Service Desk template
} else {
    log.error("No template key defined for type: ${typeKeyStr}")
    throw new Exception("Unsupported project type for template")
}
log.warn("Using template key: ${templateKey}")
...
def creationData = new ProjectCreationData.Builder()
        ...
        .withProjectTemplateKey(templateKey)  // This sets up the full template configuration
        .build()
...
// Add comment to the original issue with link to new project
...
log.warn("Added comment to issue ${issue.key} with link to new project")
